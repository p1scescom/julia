...
isexpr(@nospecialize(ex), head::Symbol, n::Int) = isexpr(ex, head) && length(ex.args) == n
...
function valid_import_path(@nospecialize ex)
    return isexpr(ex, :(.)) && length((ex::Expr).args) > 0 && all(a->isa(a,Symbol), (ex::Expr).args)
end
...
function show_unquoted(io::IO, ex::Expr, indent::Int, prec::Int, quote_level::Int = 0)
    ...
    if head === :(.) && (nargs != 2 || !is_expr(args[2], :tuple))
        if nargs == 2 && is_quoted(args[2])
            ...
            parens = !is_quoted(item) && !(item isa Symbol && isidentifier(item)) && !isexpr(item, :(.))
            parens && print(io, '(')
    ...
    elseif head === :tuple
        print(io, "(")
        if nargs > 0 && isexpr(args[1], :parameters)
    ...
    elseif head === :call && nargs >= 1
        ...
        if any(a->isexpr(a, :kw), func_args)
    ...
    elseif haskey(expr_calls, head) && nargs >= 1  # :ref/:curly/:calldecl/:(.)
        ...
        show_call(io, head, args[1], funcargslike, indent, quote_level, head !== :curly)
    ...
    elseif head === :quote && nargs == 1 && isexpr(args[1], :block)
